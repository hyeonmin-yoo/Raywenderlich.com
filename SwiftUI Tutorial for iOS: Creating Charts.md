...
여기에서는 SwiftUI가 제공하는 기능적 이점들을 알아보며 어떻게 하면 좋은 바 차트를 만들수 있는지 살펴 보겠습니다. 외부 ```HStack```은 이 차트에서 동등하게 각 바가 차지하는 공간을 나눕니다. 이러한 기능은 코드를 읽기 쉽게(readablity) 해줍니다. 각 바의 높이는 일년동안의 강우량 총량을 보여줍니다.
