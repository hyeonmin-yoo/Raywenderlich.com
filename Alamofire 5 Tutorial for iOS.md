...
여러분이 iOS앱을 개발해 본 적이 있으시다면, 아마도 네트워크를 통해 데이터에 접근해야 할 일이 있었을 것입니다. Swift의 Foundation에서 제공하는 ```URLSession```을 사용했을 수도 있습니다. ```URLSession```은 괜찮은 기능을 제공하지만, 가끔씩 사용하기에 불편하기도 합니다. 그리고 그런 이유로 이 튜토리얼이 쓰여졌습니다.

**Alamofire**는 Swift를 기반(Swift-based)으로 만들어진 HTTP 네트워킹 라이브러리입니다. Apple의 Foundation 네트워킹 계층 위에서 일반적인 네트워킹 작업을 간순화하여 멋진 인터페이스(interface)를 제공합니다.
...
* 응답받은 데이터를 Swift data 모델(model)로 변환하기 위해 Codable protocol 사용하기
